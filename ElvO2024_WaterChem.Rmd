...
## Data
- general water chemistry extracted from Aquamonitor by Ãyvind.
- historic data! (2019-2023): Combining 2023-5yr data with data from 2023 and excluding 2018
- NIVAlab bruker ikke betegnelsen SPM, bare TSM og STS. Forskjellen mellom analysene er hovedsakelig at det benyttet forskjellige filtere. STS bruker «vanlige» glassfiberfiltre: GF/C, mens TSM bruker nucleopore kapillærfilter, som jeg tror har litt mindre porestørrelser. For Elveovervåkingen bruker vi TSM som analyseparameter for SPM.
...
- some issue with moved station at MÃ¥lselv? or only historic previous data?
...
- har det no Ã¥ si om det er mellomrm mellom < og tallet?
...
- when problem uploading Ã¦, Ã¸, Ã¥: Given my R version and settings, this works for me:
...
- problem with letters Ã¦, Ã¸, Ã¥
- some stations need another name, eg. VegÃ¥rselva
```{r echo = T, results = 'hide'}
...
a2024$Station.name <- revalue(a2024$Station.name, c("Veg???rdselva"="Storelva"))
a2024$Station.name <- revalue(a2024$Station.name, c("Veg???rdselva"="Storelva"))
a2024$Station.name <- revalue(a2024$Station.name, c("Numedalsl???gen"="Numedalsl�gen"))
a2024$Station.name <- revalue(a2024$Station.name, c("Numedalslågen"="Numedalsl�gen"))
a2024$Station.name <- revalue(a2024$Station.name, c("Numedalsl???gen"="Numedalsl�gen"))
a2024$Station.name <- revalue(a2024$Station.name, c("Numedalsl???gen"="Numedalsl�gen"))
...
a2024$Station.name <- revalue(a2024$Station.name, c("M???lselva v/gml E6-brua"="M�lselva"))
a2024$Station.name <- revalue(a2024$Station.name, c("M???lselva v/gml E6-brua"="M�lselva"))
a2024$Station.name <- revalue(a2024$Station.name, c("M???lselva v/gml E6-brua"="M�lselva"))
a2024$Station.name <- revalue(a2024$Station.name, c("Målselva"="M�lselva"))
...
#five$Station.name <- revalue(five$Station.name, c("MÃ¥lselv"="M�lselva"))
...
five$Station.name <- revalue(five$Station.name, c("Numedalslågen"="Numedalsl�gen"))
five$Station.name <- revalue(five$Station.name, c("Numedalsl?en"="Numedalsl�gen"))
five$Station.name <- revalue(five$Station.name, c("M?selv"="M�lselva"))
#five$Station.name <- revalue(five$Station.name, c("M<e5>lselv"="Målselva"))
five$Station.name <- revalue(five$Station.name, c("M?selva v/gml E6-brua"="Målselva"))
five$Station.name <- revalue(five$Station.name, c("Målselva"="M�lselva"))
five$Station.name <- revalue(five$Station.name, c("Målselva"="M�lselva"))
five$Station.name <- revalue(five$Station.name, c("M?selv"="M�lselva"))
...
#bla <- subset(a23_freq_lod, Station.name=="MÃ¥lselva")
```
...
```{r}
#Blir det riktig dersom vi tar gjennomsnitt med group station.name og date? spesielt med tanke pÃ¥ flomprÃ¸vene
...
#the following averaging was not working well and i do not know why. For example for MÃ¥lselv there is only one observation > LOD, but still the average value endded up being something different. Therefore we try to first remove all zeroes. 
...
Data2 <- subset(Data1, parameter%in%c("Station.name", "Date", "TOC", "DOC", "TOTP", "TOTPP", "PO4.P", "TOTN", "NO3.N", "NH4.N", "Nitrogen.part.�g.l.N"))
...
dx24$TDN = dx24$TOTN - dx24$Nitrogen.part.�g.l.N
```
...
#fill = forcats::fct_rev(chem)))
#PLOT stacket with possibility of facet
TDN_TPN <- ggplot(subset(dfsum3x, chem %in% c("TDN", "Nitrogen.part.�g.l.N")), aes(x=Station.name, y=xmean,
                                                  fill = chem))+
  geom_col(width=0.6)+
  theme_light()+
  scale_fill_manual(values = c("lightskyblue", "dodgerblue2"),
...
THg_aveX$Station.name <- revalue(THg_aveX$Station.name, c("M?lselva"="M�lselva", "Numedalsl?en"="Numedalsl�gen", "Numedalsl?gen"="Numedalsl�gen", "Tanaelva"="Tana", "Veg?rdselva" ="Storelva", "M?selva v/gml E6-brua"="M�lselva", "Glomma ved Sarpsfoss"="Glomma", "Nidelva(Tr.heim)"="Nidelva", "Vosso(Bolstadelvi)"="Vosso", "Veg?dselva"="Storelva"))
...
```{r Naming}
Qs_22$Name <- revalue(Qs_22$Name, c("Numedalslâ gen"="NumedalslÃ¥gen", "Glomma ved Sarpsfoss"="Glomma", "Vosso (Bolstadelvi)" ="Vosso", "Tanaelva"="Tana", "Nidelva (Tr.heim)" ="Nidelva", "Mâ lselva v/gml E6-brua"="MÃ¥lselva", "	Vegâ rdselva"="Storelva", "Numedalsl?Âgen "="NumedalslÃ¥gen", "M?Âlselva"="MÃ¥lselva", "Veg?Ârdselva"="Storelva", "	
Numedalsl?Âgen"="NumedalslÃ¥gen", "NumedalslÃÂ¥gen"="NumedalslÃ¥gen"))
```
...
df.sum2x <- df.sum2 %>% 
  group_by(Year) %>% 
  mutate(Station.name = factor(Station.name, # put education levels in logical order
                               levels = c("Glomma", "Alna", "Drammenselva", "NumedalslÃ¥gen", "Skienselva",
                                         "Storelva", "Otra", "Bjerkreimselva", "Orreelva", "Vikedalselva",
                                          "Vosso", "Nausta", "Driva", "Orkla", "Nidelva", "Vefsna", "MÃ¥lselva",
                                          "Altaelva", "Tana", "Pasvikelva")))
