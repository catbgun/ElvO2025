...
subs <- subset(dfsum3, Vassdrag == c("Ekso", "Lygna", "Nidelva", "Suldalslågen"))
print(unique(subs$Vassdrag))

ss <- na.omit(dfsum3$verdi_mean)

zz <- subset(dfsum3, parameter %in% c("As", "Cr", "Cu", "Ni", "Pb", "Zn"))
zz1 <- subset(zz, Vassdrag %in% c("Ekso", "Lygna", "Nidelva", "Suldalslågen"))
...
met <- ggplot(zz2, aes(x=Vassdrag, y=verdi_mean, fill=year))  +
...
zz1_tur <- subset(zz_tur, Vassdrag %in% c("Ekso", "Lygna", "Nidelva", "Suldalslågen"))
...
kalk$Org.N = kalk$TotN - kalk$NO3 - kalk$NH4
...
kalk24_2 <- kalk24[c("Vassdrag", "year", "month", "Dato", "TOC", "DOC", "POC1", "Org.N", "TotN", "NH4", "NO3")]
...
POCDOC <- ggplot(subset(kalk24.sum2y, chem %in% c("DOC", "POC1")), aes(x=Station.name, y=xmean,
                                                  fill = forcats::fct_rev(chem)))+
  geom_col(width=0.6)+
  theme_light()+
...
N.frac_k <- ggplot(subset(kalk24.sum3, chem %in% c("NH4", "NO3", "Org.N")), aes(x=Station.name, y=xmean,
                                                  fill = chem))+
  geom_col(width=0.6)+
  theme_light()+
  scale_fill_manual(values = c("cyan2", "cyan3", "cyan4"), 
